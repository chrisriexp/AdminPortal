    ...
    <div v-if="ready" class="w-full h-screen z-20 absolute">
        ...
        <div class="w-full h-full z-10 absolute pt-[110px] pb-12 pl-[350px] pr-[50px] overflow-y-scroll">
            <div class="w-ful h-fit grid gap-8">
                ...
                <div class="w-full h-fit grid p-6 rounded-[4px] border-[1px] border-custom-black border-opacity-10 shadow-newdrop">
                    <div class="w-full h-fit grid">
                        <p class="text-[20px] text-custom-black font-semibold">Submission Conversion</p>
                        <p class="text-[16px] text-custom-black opacity-50">Here is the all time submission conversion rate,</p>
                    </div>
    
                    <div class="w-fit h-fit mx-auto mt-8 flex items-center gap-12 flex-wrap">
                        <div v-for="(rep, index) in data.conversion" :key="index" class="w-fit h-fit grid justify-items-center">
                            <p>{{ rep.name }}</p>
                            <Knob v-model="rep.conv" valueTemplate="{value}%" class="mx-auto" :size="175" :strokeWidth="5" disabled  />
                        </div>
                    </div>
                </div>
