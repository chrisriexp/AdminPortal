        ...
        <div class="w-[1176px] h-[691px] m-auto p-6 grid bg-sidebar-bg rounded-[4px] relative shadow-customdrop">
            ...
            <div class="w-full h-fit grid grid-cols-5">
                <div class="w-full h-full flex items-center">
                    <p class="text-[24px] text-custom-black font-semibold">{{ active == 'tasks' ? 'Reminders' : (active == 'notifications' ? 'Notifications' : 'RocketAI (beta)') }}</p>
                </div>
                ...
                <div class="w-full h-full col-span-3 flex items-center">
                    ...
                    <button @click="active = 'rocketAI'" :class="active == 'rocketAI' ? 'text-white bg-custom-purple' : 'text-custom-black opacity-50'" class="w-fit h-fit mx-auto flex items-center gap-4 text-[16px] font-medium px-4 py-[5px] rounded-[4px]"><Icon icon="simple-icons:openai" height="24" class="m-auto" />Rocket AI</button>
                </div>
...
export default {
    ...
    async created(){
        this.settingVue = defineAsyncComponent(() => import(`../components/quick_tools/${this.active}.vue`))
    },
    watch: {
