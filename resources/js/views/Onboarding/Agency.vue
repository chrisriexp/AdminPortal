<template>
    ...
    <div v-if="ready" class="w-full h-screen z-20 absolute">
        ...
        <div class="w-[305px] h-full grid bg-sidebar-bg z-20 absolute">
            <div class="w-full h-fit max-h-[700px] grid gap-2 mt-[100px] px-6 overflow-y-scroll text-[16px]">
                <div class="w-full h-fit grid gap-4 my-8 text-[#787878] font-medium">
                    <div v-if="this.general.completed && !this.general.approved" @click="$router.replace({params: {category: 'approve'}})" :class="category == 'approve' ? 'text-custom-black bg-[#F1F1F1]' : ''" class="w-full h-fit px-4 py-2 flex items-center gap-4 rounded-[4px] hover:bg-[#F1F1F1] cursor-pointer"><Icon icon="bxs:badge-check" height="24" /> Approval Checklist</div>
                    ...
                    <div v-if="this.general.approved" @click="$router.replace({params: {category: 'carriers'}})" :class="category == 'carriers' ? 'text-custom-black bg-[#F1F1F1]' : ''" class="w-full h-fit px-4 py-2 flex items-center gap-4 rounded-[4px] hover:bg-[#F1F1F1] cursor-pointer"><Icon icon="icon-park-solid:id-card" height="24" /> Carrier Credentials</div>
...
export default {
    ...
    async created(){
        ...
        .then(response => {
            ...
            this.general.follow_up_date = moment(this.general.follow_up_date).format('MM/DD/YYYY')
        })
